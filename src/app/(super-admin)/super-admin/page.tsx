...
export default function SuperAdminPage() {
  ...
  // Database Scan Cleanup State
  const [cleaningScans, setCleaningScans] = useState(false);

  const handleRunManualScanCleanup = async () => {
    if (!confirm('This will safely delete raw QR scan records strictly older than 60 days. Current billing cycle data, active quotas, and customer feedback will NOT be touched. Proceed?')) {
      return;
    }
    try {
      setCleaningScans(true);
      const res = await fetch('/api/super-admin/cleanup', {
        method: 'POST',
      });
      const data = await res.json();
      if (res.ok) {
        setActionSuccess(data.message || 'Old scan logs cleaned successfully!');
        setTimeout(() => setActionSuccess(''), 4000);
        fetchData();
      } else {
        alert(data.error || 'Failed to clean old scans');
      }
    } catch (err: any) {
      alert(err.message);
    } finally {
      setCleaningScans(false);
    }
  };
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        {activeTab === 'CLIENTS' && (
          <div className="space-y-6">
            ...
            {/* Database Health & 60-Day Scan Storage Cleaner */}
            <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-indigo-950 text-white rounded-3xl p-5 sm:p-6 shadow-lg flex flex-col md:flex-row md:items-center justify-between gap-4 border border-slate-700">
              <div className="flex items-start gap-3.5">
                <div className="w-10 h-10 rounded-2xl bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 flex items-center justify-center shrink-0 mt-0.5">
                  <Database className="w-5 h-5 text-indigo-400" />
                </div>
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="text-sm font-black text-white tracking-wide">
                      Database Health & Scan Storage Cleaner
                    </h3>
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-black bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 uppercase tracking-wider">
                      Auto-Retention: 60 Days Active
                    </span>
                  </div>
                  <p className="text-xs text-slate-300 mt-1 leading-relaxed max-w-2xl">
                    Strict Protection: QR scan records strictly older than 60 days are safely pruned to keep PostgreSQL lean. Current billing cycle scans, active limits, and customer feedback remain <strong className="text-emerald-400 font-bold">100% untouched & safe</strong>.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 shrink-0">
                <button
                  type="button"
                  onClick={handleRunManualScanCleanup}
                  disabled={cleaningScans}
                  className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold shadow-md transition disabled:opacity-50"
                  title="Safely clean scan analytics strictly older than 60 days"
                >
                  <Trash2 className="w-4 h-4 text-indigo-200" />
                  <span>{cleaningScans ? 'Cleaning Database...' : '🧹 Clean 60+ Days Old Scans Now'}</span>
                </button>
              </div>
            </div>
